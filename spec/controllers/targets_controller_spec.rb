...
RSpec.describe TargetsController, type: :controller do
  ...
  describe '#create' do
    context 'ログイン' do
      it '登録可能か' do
        sign_in @user
        expect { FactoryBot.create(:target, user_id: @user.id.to_s) }.to change(@user.targets, :count).by(1)
      end
    end
  ...
  describe '#destroy' do
    context 'ログイン' do
      context '登録したユーザー' do
        it '削除可能か' do
          sign_in @user
          @usertarget = FactoryBot.create(:target, user_id: @user.id.to_s) 
          expect {
      ...
      context '他のユーザー' do
        it '削除不可能か' do
          ...
          @another_user_target = FactoryBot.create(:target, user_id: @another_user.to_s) 
          expect {
