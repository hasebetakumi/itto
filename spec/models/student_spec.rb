  ...
  describe '生徒登録' do
    context 'バリデーション' do
      ...
      it 'school_idがないと登録できない' do
        expect(FactoryBot.build(:student, school_id: '')).not_to be_valid
      end
        
      it 'gradeがないと登録できない' do
        expect(FactoryBot.build(:student, grade: '')).not_to be_valid
      end
        
      it 'student_type_idがないと登録できない' do
        expect(FactoryBot.build(:student, student_type_id: '')).not_to be_valid
      end
         
      it 'classifyingがないと登録できない' do
        expect(FactoryBot.build(:student, classifying: '')).not_to be_valid
      end
        
      it 'family_nameがないと登録できない' do
        expect(FactoryBot.build(:student, family_name: '')).not_to be_valid
      end
        
      it 'given_nameがないと登録できない' do
        expect(FactoryBot.build(:student, given_name: '')).not_to be_valid
      end
        
      it 'family_name_kanaがないと登録できない' do
        expect(FactoryBot.build(:student, family_name_kana: '')).not_to be_valid
      end
        
      it 'given_name_kanaがないと登録できない' do
        expect(FactoryBot.build(:student, given_name_kana: '')).not_to be_valid
      end
    end
